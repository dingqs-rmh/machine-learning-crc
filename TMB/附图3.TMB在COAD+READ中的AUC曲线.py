...
# 创建一个PDF文件保存AUC曲线
with PdfPages('/home/xkj/project/TMB/plots/附图3.pdf') as pdf_auc:

    # 创建一个新的图形来容纳所有折叠的AUC曲线
    ...
    for i, (train_idx, test_idx) in enumerate(cv.split(X, y)):
        ...
        # 预测概率
        final_test_pred_prob = best_model.predict_proba(X_test_selected)[:, 1]

        # 计算ROC曲线和AUC值
        fpr, tpr, thresholds = roc_curve(y_test, final_test_pred_prob)
        auc_value = auc(fpr, tpr)

        # 绘制AUC曲线
        ax = axes[i // 3, i % 3]  # 根据折叠索引决定位置
        ax.plot(fpr, tpr, label=f'AUC = {auc_value:.3f}')
        ax.plot([0, 1], [0, 1], '--', color='gray')  # 随机分类器的对角线
...
performance_df.to_csv('/home/xkj/project/TMB/metrics/performance_metrics_auc.csv', index=False)

print("交叉验证的AUC曲线已保存为PDF文件，AUC值已保存为CSV文件。")
