setwd("~/project/PIK3CA/data")
rm(list = ls())

group1 <- read.table("READ_PIK3CA.txt", header = T, sep = '\t')
# 取并集
union_samples <- unique(c(group1$Sample))

# 创建新的数据框
group <- data.frame(Sample = union_samples)
#group <- read.table("READ_KRAS_type.txt", header = T, sep = '\t')
...
Target <- ifelse(rownames(transposed_tpm_data) %in% group$Sample, 1, 0)

# 将Target列移到首列
final_tpm_data <- cbind(Target, transposed_tpm_data)

# 查看结果
head(final_tpm_data)

# 保存为CSV文件
write.csv(final_tpm_data, "READ_PIK3CA_with_SampleID_01A.csv", row.names = TRUE, quote = F)
