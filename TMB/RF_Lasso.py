...
# 使用GridSearchCV进行交叉验证超参数搜索
grid_search = GridSearchCV(estimator=rf_model, param_grid=param_grid, scoring='roc_auc', cv=5, n_jobs=-1, verbose=2)
...
# 使用cross_val_score查看每一折的表现
cv_results = cross_val_score(best_model, X_train_resampled, y_train_resampled, cv=5, scoring='roc_auc')
print(f'每折交叉验证的AUC得分: {cv_results}')
print(f'交叉验证AUC的平均值: {cv_results.mean():.2f} ± {cv_results.std():.2f}')
