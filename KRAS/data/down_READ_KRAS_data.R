...
group1 <- read.table("READ_KRAS_type.txt", header = T, sep = '\t')
group2 <- read.table("READ_BRAF_type.txt", header = T, sep = '\t')
group3 <- read.table("READ_NRAS_type.txt", header = T, sep = '\t')
# 取并集
...
# 保存为CSV文件
write.csv(final_tpm_data, "READ_RAS_RAF_with_SampleID.csv", row.names = TRUE, quote = F)
