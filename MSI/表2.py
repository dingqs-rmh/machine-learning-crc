...
def save_metrics_to_csv(test_files, model_dir="/home/xkj/project/MSI/model/", output_csv="table2.csv"):
    """
...
# 给定的文件顺序列表
file_order = [
    "TCGA_STAD.csv", "TCGA_UCEC.csv"
]
test_files = [os.path.join(data_directory, file) for file in os.listdir(data_directory) if file.startswith('TCGA') and file.endswith('.csv')]
test_files_sorted = sorted(test_files, key=lambda x: file_order.index(os.path.basename(x)))

# 输出CSV路径
output_csv = "/home/xkj/project/MSI/test_result/表2.csv"
